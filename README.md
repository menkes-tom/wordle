...
1. **Install dependencies:** 
   ...
   ```
2. **Simulate a full run of the dictionary:** 
   ```bash
   python -m scripts.simulate_all_words
   ```
3. **Analyze starting words:** 
   ```bash
   python -m scripts.analyze_first_word
   ```
4. **Play an interactive, guided game:** 
   ```bash
   python -m scripts.game_simulation
   ```
...
* [Wordle original](https://www.nytimes.com/games/wordle/index.html) - Play the classic 1 word per day game
* [Wordle spin-off](https://wordleunlimited.org/) - Play an unlimited game
...
#### So how do we verify these results? 
Since the data set is complete, we can start playing wordle using the word "slate" and check to see how it goes. but where is the fun in that? 
